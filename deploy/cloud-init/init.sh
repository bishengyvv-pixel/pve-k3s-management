...
finalize_and_cleanup() {
    ...
    if [ "${K3S_TYPE}" = "control_node" ]; then
        # 获取 Master IP 和 Token
        local K3S_MASTER_IP_FOUND
        K3S_MASTER_IP_FOUND=$(hostname -I | awk '{print $1}')
        # 假设 k3s/create_node.sh 已经完成了 k3s 的安装
        local K3S_TOKEN
        K3S_TOKEN=$(cat /var/lib/rancher/k3s/server/token) || true # 允许失败，如果 token 不存在
